--- 在实体上延迟执行一次任务（自动 core.Wrap，异常不会拖垮主机）。
--- 对应引擎 `inst:DoTaskInTime`；调用方须保证 inst 有效。
---
--- @class CoreTaskHost
--- @field DoTaskInTime fun(self: CoreTaskHost, time: number, fn: function, ...: any): PeriodicTask
--- @field DoPeriodicTask fun(self: CoreTaskHost, time: number, fn: function, ...: any): PeriodicTask
---
...
core.SetTimeout = function(inst, fn, wait)
  if inst == nil then
    return nil
  end
