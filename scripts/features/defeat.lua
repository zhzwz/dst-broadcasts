...
local function ResolveDamageSource(afflicter)
  ...
  if owner ~= nil then
    ...
    local summon_name = core.GetPrefabDisplayName(prefab) or GetEntityName(afflicter) or i18n.boss_damage_other
  ...
  if afflicter ~= nil and afflicter:IsValid() then
    ...
    local name = GetEntityName(afflicter) or core.GetPrefabDisplayName(prefab) or i18n.boss_damage_other
    return key, name
  end
...
local function WatchBossDamage(inst, get_bucket)
  inst:ListenForEvent("healthdelta", core.Wrap(function(_, data)
...
local function AnnounceDamageRanking(bucket)
  ...
  local sep = i18n.symbol.enumeration
...
for prefab, name in pairs(NONLETHAL_BOSSES) do
  AddPrefabPostInit(prefab, core.Wrap(function(inst)
    ...
    inst:ListenForEvent("minhealth", core.Wrap(function(_, data)
...
AddPrefabPostInit("vault_pillar_guard", core.Wrap(function(inst)
