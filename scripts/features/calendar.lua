...
local function AnnounceCalendar()
  ...
  if remaining <= 0 then
    ...
    core.Announce(string.format(template, day, season_name, day_in_season, next_name))
    return
  end
  ...
  core.Announce(string.format(template, day, season_name, day_in_season, next_name, remaining))
end
