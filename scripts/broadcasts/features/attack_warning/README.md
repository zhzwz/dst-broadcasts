...
| `deerclops_warning_enabled`    | 巨鹿：倒计时 + 现身（含晶体巨鹿） |
| `bearger_warning_enabled`      | 熊獾：倒计时 + 现身（含装甲熊獾） |
...
| `hassler.lua`   | 巨鹿 / 熊獾（`worldsettingstimer` + 现身）               |

## 倒计时档位

现实时间：8 / 4 / 2 / 1 分钟，以及 30 / 10 / 5 秒。
文案键：`attack_time`、`durations[秒]`；袭击开始：`attack_started`；现身：`boss_appeared`。

与早报无关；不写持久化状态，重载后阈值标记重置。
